...
class BST<K extends Comparable<K>, E> implements Dictionary<K, E> {
  ...
  protected void rangehelp(BSTNode<K, E> v, ArrayList<E> elts, K a, K b) {
    ...
    int initialCompare = v.key().compareTo(a);
    int finalCompare = v.key().compareTo(b);
    if (initialCompare >= 0){
      rangehelp(v.left(), elts, a,b);
    }
    if (initialCompare >= 0 && finalCompare <= 0){
      elts.add(v.element());
    }
    if (finalCompare <= 0){
  ...
  public void levelorderElements(BSTNode<K, E> v, ArrayList<E> elts) {

    Queue<BSTNode<K, E>> queue = new LinkedList<BSTNode<K, E>>();
    queue.add(v); //Add the root;

    while (!queue.isEmpty()) {
        BSTNode<K, E> currentNode = queue.poll();
        elts.add(currentNode.element()); //Adds the letter of the current node.
